...
data = {
    "employee_id": [f"E{i}" for i in range(1, 21)],
    "shift_timing": ["Morning", "Afternoon", "Night"] * 7,
    "task_completion_rate": [55, 60, 68, 75, 80, 85, 90, 92, 95, 98, 60, 58, 65, 70, 80, 85, 90, 88, 95, 97],
    "overtime_hours": [5, 10, 2, 8, 6, 7, 4, 12, 3, 0, 8, 5, 13, 7, 12, 6, 9, 3, 5, 9]
}
...
df_line = df.groupby(pd.Grouper(key='employee_id', freq='W')).mean().reset_index()
fig_line = px.line(df_line, x='employee_id', y='task_completion_rate', title='Weekly Trend of Task Completion Rate')
